...
import { signZipBufferForPackage } from '../signature/index'
...
function validateRpkOrSrpkFile(subPackageBufferList, subPackages) {
  ...
  subPackageBufferList.forEach((subPackageBufferItem, i) => {
    ...
    if (size > SINGLE_PKG_SIZE) {
      colorconsole.warn(
        `### App Loader ### 每个分包大小不能大于 ${SINGLE_PKG_SIZE /
          1024} KB, '${rpkBuildFileName}' 已超出`
      )
    }
  })
  // 检查分包总和大小
 ...
 */
async function buildProjectAndOutput(fullPackage, subPackages, signConfig, disableStreamPack) {
  // Step1. 生成整包rpk
