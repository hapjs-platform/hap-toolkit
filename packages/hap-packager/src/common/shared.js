...
function populateWidgetFields(widgetsObj) {
  Object.keys(widgetsObj).forEach((routePath) => {
    ...
    if (!conf.type) {
      conf.type = `js`
      colorconsole.warn(
        `WARN: manifest.json 文件中 widgets 字段 ${routePath} 缺少 type 属性，默认设置为 js`
      )
    }
    ...
    if (conf.type === 'js') {
      ...
      if (conf.minCardPlatformVersion) {
        ...
        throw new Error(
          `manifest.json 文件中 widgets 字段中，${routePath} 缺少 minCardPlatformVersion 字段`
        )
      }
    }
    if (conf.type === 'lite' && !conf.minCardPlatformVersion) {
      throw new Error(
        `manifest.json 文件中 widgets 字段中，轻卡 ${routePath} 缺少 minCardPlatformVersion 字段`
      )
    }
  })
}
